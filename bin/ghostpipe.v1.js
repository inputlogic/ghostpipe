...
const createSession = (hostConfig, signalingServer) => {
  ...
  if (host.startsWith('http://') || host.startsWith('https://')) {
    connectUrl = `${host}/gui?pipe=${pipeId}&signaling=${encodedSignaling}`
  } else {
...
const createDiffSession = (hostConfig, baseBranch, headBranch, signalingServer) => {
  const name = hostConfig.name || 'Default'
  const host = hostConfig.host
  const allowedFiles = hostConfig.files || null
  ...
  // Check if head branch is the current working branch
  let isWorkingDirectory = false
  try {
    const currentBranch = execSync('git branch --show-current').toString().trim()
    isWorkingDirectory = (headBranch === currentBranch)
  } catch {
    // If we can't determine current branch, assume not working directory
  }
  ...
  try {
    ...
    // If using working directory, include unstaged changes
    let changedFilesOutput
    if (isWorkingDirectory) {
      // Get both committed changes and working directory changes
      const committedChanges = execSync(
        `git diff --name-only ${baseBranch}...${headBranch}`
      ).toString().trim()
      const workingChanges = execSync(
        `git diff --name-only ${baseBranch}`
      ).toString().trim()
      
      // Combine and deduplicate
      const allChanges = new Set([
        ...(committedChanges ? committedChanges.split('\n') : []),
        ...(workingChanges ? workingChanges.split('\n') : [])
      ])
    ...
    if (!changedFilesOutput) {
      ...
      // Filter files based on allowedFiles if specified
      if (allowedFiles) {
        changedFiles = changedFiles.filter(file => allowedFiles.includes(file))
        if (changedFiles.length === 0) {
          console.log(`[${name}] No matching files in the diff`)
          metadata.set('changedFiles', [])
          return
        }
      }
      ...
      if (isWorkingDirectory && VERBOSE) console.log('  (including working directory changes)')
      ...
      changedFiles.forEach(file => {
        ...
        // For head branch, use working directory if it's the current branch
        if (isWorkingDirectory) {
          try {
            ...
            if (fs.existsSync(filePath)) {
              ...
              headFiles.set(file, '')
              if (VERBOSE) console.log(`  ${file}: deleted in working directory`)
            }
          } catch (error) {
            // Error reading file, treat as empty
            headFiles.set(file, '')
            if (VERBOSE) console.log(`  ${file}: error reading from working directory`)
          }
        } else {
          try {
            // Get file content from head branch in git
  ...
  // Generate diff URL
  const diffUrl = `${host}/diff?pipe=${pipeId}&signaling=${encodedSignaling}`
  console.log(`${name}: ${diffUrl}`)
  if (VERBOSE) console.log(`  Comparing: ${baseBranch} → ${headBranch}${isWorkingDirectory ? ' (including unstaged changes)' : ''}`)
  
  // Watch for local filesystem changes if using working directory
  const watchedFiles = new Map()
  const watchTimeout = new Map()
  ...
  if (isWorkingDirectory) {
    ...
    const watchFile = (relativePath) => {
      ...
      if (!fs.existsSync(fullPath)) {
        if (VERBOSE) console.log(`[${name}] Skipping watch for non-existent file: ${relativePath}`)
        return
      }
      
      try {
        if (VERBOSE) console.log(`[${name}] Watching file: ${relativePath}`)
        ...
        const createWatcher = () => {
          const watcher = fs.watch(fullPath, (eventType) => {
            ...
            watchTimeout.set(relativePath, setTimeout(() => {
              if (VERBOSE) console.log(`[${name}] Detected ${eventType} for: ${relativePath}`)
              // Handle both 'change' and 'rename' events (macOS reports 'rename' for modifications)
              if (eventType === 'change' || eventType === 'rename') {
                try {
                  if (!fs.existsSync(fullPath)) {
                    if (VERBOSE) console.log(`[${name}] File no longer exists: ${relativePath}`)
                    return
                  }
                  ...
                  if (VERBOSE) console.log(`[${name}] File size - old: ${currentContent ? currentContent.length : 0}, new: ${content.length}`)
                  ...
                  if (content !== currentContent) {
                    ...
                    if (VERBOSE) console.log(`[${name}] Updated file in diff: ${relativePath} (${content.length} bytes)`)
                  } else {
                    if (VERBOSE) console.log(`[${name}] File content unchanged: ${relativePath}`)
                  }
                  
                  // Re-establish watcher after rename event (macOS issue)
                  if (eventType === 'rename') {
                    if (VERBOSE) console.log(`[${name}] Re-establishing watcher for: ${relativePath}`)
                    if (watchedFiles.has(relativePath)) {
    ...
    if (VERBOSE) console.log(`[${name}] Setting up file watchers for ${changedFiles.length} files in working directory`)
    changedFiles.forEach(file => {
      watchFile(file)
    })
  }
...
const main = () => {
  ...
  if (parsed.command === 'diff') {
    ...
    if (!config.hosts || !Array.isArray(config.hosts) || config.hosts.length === 0) {
      console.error('Error: No hosts configured. Please create a .ghostpipe.json config file with hosts array.')
      process.exit(1)
    }
    
    if (VERBOSE) console.log(`Starting diff mode: ${baseBranch} → ${headBranch}`)
    console.log(`\nComparing: ${baseBranch} → ${headBranch}${currentBranch === headBranch ? ' (including unstaged changes)' : ''}`)
    
    const cleanupFunctions = []
    
    // Create a diff session for each host
    config.hosts.forEach((hostConfig) => {
      const cleanup = createDiffSession(hostConfig, baseBranch, headBranch, signalingServer)
      cleanupFunctions.push(cleanup)
    })
    
    console.log('\nAll diff pipes are running. Press Ctrl+C to stop.')
    
    // Handle cleanup on exit
    process.on('SIGINT', () => {
      console.log('\nShutting down all Ghostpipe diff connections...')
